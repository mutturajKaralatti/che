...
'use strict';
import {
  IEnvironmentManagerMachine, IEnvironmentManagerMachineServer, IEnvironmentRuntimeMachineServer
} from './environment-manager-machine';
...
export class EnvironmentManager {
  ...
  getServers(machine: IEnvironmentManagerMachine): {[serverName: string]: IEnvironmentManagerMachineServer} {
    ...
    Object.keys(machine.servers).forEach((serverName: string) => {
      machine.servers[serverName].systemScope = false;
    });
    ...
    Object.keys(machine.runtime.servers).forEach((serverName: string) => {
      let runtimeServer: IEnvironmentRuntimeMachineServer = machine.runtime.servers[serverName];

      if (machine.servers[runtimeServer.ref]) {
        machine.servers[runtimeServer.ref].runtime = runtimeServer;
      } else {
        machine.servers[runtimeServer.ref] = {
          systemScope: true,
  ...
  setServers(machine: IEnvironmentManagerMachine, _servers: {[serverRef: string]: IEnvironmentManagerMachineServer}): void {
    ...
    Object.keys(_servers).forEach((serverName: string) => {
      ...
      if (_servers[serverName].systemScope) {
